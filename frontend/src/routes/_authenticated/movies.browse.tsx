...
import { useMoviesAll } from "@/hooks/use-catalog";
...
export const Route = createFileRoute("/_authenticated/movies/browse")({
  validateSearch: (search) => ({
    ...
    availableOnly:
      search.availableOnly === true || search.availableOnly === "true",
    genres: parseGenres(search.genres),
  }),
  component: MoviesBrowsePage,
});
...
function MoviesBrowsePage() {
  ...
  useEffect(() => {
    ...
    if (hasChanged) {
      navigate({ to: "/movies/browse", search: nextSearch, replace: true });
      queryClient.removeQueries({ queryKey: ["movies-all"] });
    }
  }, [
  ...
  const items = useMemo(() => {
    ...
    return pages.filter((movie) => {
      if (filters.availableOnly && !movie.available) return false;
      if (movie.voteAverage < filters.minRating) return false;
  ...
  const toMediaItem = (movie: EnrichedMovie): MediaItem => {
    const streamOptions = movie.streams.map((stream) => ({
      name: stream.name,
    ...
    return {
      ...
      cover: movie.posterPath
        ...
        : movie.streams[0]?.logoUrl ?? "",
      backdrop: movie.backdropPath
        ? `${TMDB_IMAGE_BASE_URL}${movie.backdropPath}`
        : undefined,
      streamUrl: movie.streams[0]?.streamUrl ?? "",
      streamOptions,
    ...
    <div className="pt-24 pb-12 px-4 md:px-12">
      <div className="flex flex-wrap items-center justify-between gap-4 mb-4">
        ...
        <h1 className="text-2xl md:text-3xl font-bold text-foreground flex-1">
          {search.group ? decodeURIComponent(search.group) : "Todos os filmes"}
        </h1>
        <button
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="mt-2 space-y-1">
                          ...
                          <AvailableBadge available={movie.available} />
                        </div>
                      </button>
                    );
                  })}
